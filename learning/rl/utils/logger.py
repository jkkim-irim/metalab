...
class Logger:
    ...
    def __init__(
        ...
        # Episodic reward sums are reported PER STEP (divided by this), the same normalization the env's own
        # per-term Reward/<name> series uses, so Train/mean_reward is their sum and not a number 300x larger.
        self.max_episode_length = max(1, int(max_episode_length))
        self.gpu_world_size = gpu_world_size
        self.device = device
        self.git_status_repos = [learning.rl.__file__]
        self.tot_timesteps = 0
        self.tot_time = 0

        # Create buffers
        self.ep_extras = []
        # Last value this logger actually WROTE, per metric key. `ep_extras` is cleared at the end of every
        # log(), so a post-log consumer (the per-checkpoint record hook) has no other way to read the
        # iteration's aggregated metrics — and reading them back off `wandb.run.summary` does not work:
        # measured, a key logged moments earlier still comes back absent there.
    ...
    def init_logging_writer(self) -> None:
        ...
        if self.log_dir is not None and not self.disable_logs:
            ...
            if self.logger_type == "wandb":
                from learning.rl.utils.wandb_writer import WandbSummaryWriter
    ...
    def log(
        ...
        if self.writer is not None:
            ...
            extras_string = ""
            if self.ep_extras:
                reward_total = None  # sum of all Episode_Reward/* term rates (the single total curve)
                ...
                for key in self.ep_extras[0]:
                    ...
                    if "/" in key:
                        ...
                        self.last_metrics[key] = float(value)
                        extras_string += f"""{f"{key}:":>{pad}} {value:.4f}\n"""
                        if key.startswith("Episode_Reward/"):
                            reward_total = float(value) + (reward_total or 0.0)
                    else:
                        self.writer.add_scalar("Episode/" + key, value, it)  # type: ignore
                        self.last_metrics["Episode/" + key] = float(value)
                        extras_string += f"""{f"Mean episode {key}:":>{pad}} {value:.4f}\n"""
                # The SUM of the per-term rates = the total episodic reward under the same normalization
                # (episodic sum × dt ÷ max_episode_length_s) — restores the single aggregate curve next to
                # the per-component breakdown (terms are registered separately so each logs its own curve).
                if reward_total is not None:
                    self.writer.add_scalar("Episode_Reward/total", reward_total, it)  # type: ignore
                    extras_string += f"""{"Episode_Reward/total:":>{pad}} {reward_total:.4f}\n"""
            ...
            if len(self.rewbuffer) > 0:
                ...
                mean_reward = statistics.mean(self.rewbuffer) / self.max_episode_length
    ...
    def stop_logging_writer(self, exit_code: int = 0) -> None:
        """Stop the logging writer. ``exit_code`` (wandb only): 0 = finished, 255 = killed (Stop/SIGINT)."""
        if self.writer is not None and self.logger_type == "wandb":
            self.writer.stop(exit_code=exit_code)  # type: ignore
        elif self.writer is not None and self.logger_type == "neptune":
