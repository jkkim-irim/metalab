...
def resolve_callable(callable_or_name: type | Callable | str) -> Callable:
    ...
    - Simple name: for example ``"PPO"`` or ``"ActorCritic"`` (searched within ``learning.rl``).
    ...
    Raises:
        ...
        ValueError: If a simple name cannot be found in learning.rl packages.
    """
    ...
    # Simple name - look for it in learning.rl
    for _, module_name, _ in pkgutil.iter_modules(learning.rl.__path__, "learning.rl."):
        ...
        if default_set_name not in obs_groups:
            if default_set_name in obs:
                ...
                raise ValueError(
                    ...
                    f" Please refer to `learning.rl.utils.resolve_obs_groups()` for information on how to configure the"
