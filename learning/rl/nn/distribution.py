...
class PerBlockGaussianDistribution(GaussianDistribution):
    """Gaussian with a per-block learnable std (SAPG ``sigma='coef_cond'``).

    The std is a ``[num_blocks, output_dim]`` parameter; each row of a batch selects its block's std by a
    ``block_ids`` index (see :meth:`update_with_blocks`). All other behaviour (sample / log_prob / entropy /
    params / kl_divergence) is inherited from :class:`GaussianDistribution` and operates on the per-row Normal,
    so PPO's ratio and adaptive-KL math are unchanged. With ``num_blocks == 1`` it is numerically identical to
    :class:`GaussianDistribution` (a single std row broadcast to every sample).
    """
    ...
    def __init__(
        ...
        # Skip GaussianDistribution.__init__ (which builds a [output_dim] std); build a per-block std instead.
        Distribution.__init__(self, output_dim)
        self.std_type = std_type
        self.num_blocks = num_blocks

        if std_type == "scalar":
            self.std_param = nn.Parameter(init_std * torch.ones(num_blocks, output_dim))
        elif std_type == "log":
            self.log_std_param = nn.Parameter(torch.log(init_std * torch.ones(num_blocks, output_dim)))
        else:
            raise ValueError(f"Unknown standard deviation type: {std_type}. Should be 'scalar' or 'log'.")

        # Internal torch distribution (populated by update_with_blocks())
        self._distribution: Normal | None = None
    ...
    def update(self, mlp_output: torch.Tensor) -> None:
        """Fallback that treats every row as block 0.

        ``SAPGActor.forward`` calls :meth:`update_with_blocks` for the real per-block std; this override only
        exists so that any accidental block-agnostic call path stays well-defined.
        """
        block_ids = torch.zeros(mlp_output.shape[0], dtype=torch.long, device=mlp_output.device)
        self.update_with_blocks(mlp_output, block_ids)

    def update_with_blocks(self, mlp_output: torch.Tensor, block_ids: torch.Tensor) -> None:
        """Build the per-row Normal, selecting each row's std by its block id.

        Args:
            mlp_output: MLP mean output, shape ``[batch, output_dim]``.
            block_ids: Long tensor of block ids, shape ``[batch]``.
        """
        mean = mlp_output
        if self.std_type == "scalar":
            std = self.std_param[block_ids]
        else:
            std = torch.exp(self.log_std_param[block_ids])
        self._distribution = Normal(mean, std)
