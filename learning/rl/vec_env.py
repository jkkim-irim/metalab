...
class VecEnv(ABC):
    ...
    def step(self, actions: torch.Tensor) -> tuple[TensorDict, torch.Tensor, torch.Tensor, dict]:
        ...
        Observations:
            ...
            The observation sets (keys of the `obs_groups` dictionary) currently used by learning.rl are:
            ...
            `learning/rl/utils/utils.py`, which provides detailed information on the expected configuration.

        Extras:
            The extras dictionary includes metrics such as the episode reward, episode length, etc. The following
            dictionary keys are used by learning.rl:
