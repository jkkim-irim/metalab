...
#
# The PPO trainer here (this file + on_policy_runner / rollout_storage / models / nn / utils) is
# adapted from rsl_rl (rsl-rl-lib v5.0.1, BSD-3-Clause, © ETH Zürich & NVIDIA), vendored + owned in-repo.
...
from learning.rl.extensions import (
...
from learning.rl.utils import resolve_callable, resolve_obs_groups, resolve_optimizer
...
class PPO:
    ...
    def __init__(
        ...
        # A single constant on the ENVIRONMENT reward, applied where rl_games applies its
        # `reward_shaper.scale_value`: it conditions the critic's target range without touching the reward
        # function's own units. Advantages are batch-normalized, so the policy gradient is invariant to it;
        # what it moves is the scale `value_loss_coef` and the value branch of `clip_param` are sized for.
        # The value bootstrap below is NOT scaled — it is already in the critic's (scaled) units.
    ...
    def process_env_step(
        ...
        self.transition.rewards = rewards * self.reward_scale       # a new tensor — the caller still logs raw
    ...
    def construct_algorithm(cls, obs: TensorDict, env: VecEnv, cfg: dict, device: str) -> PPO:
        """Construct the algorithm. Subclasses may override :meth:`_make_storage` to swap the storage class."""
        ...
        # Initialize the storage (subclass hook: SAPG swaps in SAPGRolloutStorage)
        storage = cls._make_storage(env, cfg, obs, device)
    ...
    def _make_storage(cls, env: VecEnv, cfg: dict, obs: TensorDict, device: str) -> RolloutStorage:
        """Build the rollout storage. Overridden by SAPG to return a block-aware storage."""
    ...
    def _entropy_loss(self, entropy: torch.Tensor, batch: RolloutStorage.Batch) -> torch.Tensor:
        """Entropy bonus subtracted from the loss — the uniform PPO term. Overridden by SAPG to weight
        each sample by its exploration block's coefficient."""
