...
from learning.rl.nn import HiddenState
...
class RolloutStorage:
    ...
    def _save_hidden_states(self, hidden_states: tuple[HiddenState, HiddenState]) -> None:
        ...
        # inference_mode(False) around the ALLOCATION: the rollout runs inside torch.inference_mode (see
        # OnPolicyRunner.learn), and a tensor created there is an INFERENCE tensor — passing one to the RNN
        # during the update raises "Inference tensors cannot be saved for backward", which made recurrent
        # policies unusable. Copying an inference tensor INTO a normal buffer is fine, so only the buffer has
        # to be created outside; the per-step copies below stay in inference mode.
