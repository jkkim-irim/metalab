...
from learning.rl.nn import MLP, EmpiricalNormalization, HiddenState
from learning.rl.nn.distribution import Distribution
from learning.rl.utils import resolve_callable, unpad_trajectories
...
class MLPModel(nn.Module):
    ...
    def forward(
        ...
        ..note::
            ...
            outputs by default.
        """
        # Get MLP input latent (from the possibly time-padded TensorDict observations)
        latent = self.get_latent(obs, masks, hidden_state)
        # If observations are padded for recurrent training but this model is non-recurrent, unpad the
        # concatenated LATENT tensor rather than the obs TensorDict. unpad_trajectories operates on
        # tensors, and since get_latent is per-timestep this is equivalent — and, unlike unpadding the
        # obs directly, it works with TensorDict observations (the RNN-actor + MLP-critic case).
