...
from learning.model.qwen3vl.pretrained_base import PretrainedConfig
...
class Gr00tN1d7Config(PretrainedConfig):
    ...
    # Simple auxiliary future torque objective. This notmulti variant does not
    # add torque tokens to the DiT input; it decodes future torque from the
    # existing action-token hidden states.
    future_torque_state_key: str | None = None
    future_torque_target_key: str | None = None
    future_torque_loss_weight: float = 0.0
    future_torque_dim: int | None = None
    reset_system1_weights: bool = False
    """If True, load the pretrained VLM/backbone from the checkpoint but
    reinitialize the motor-side System1 modules after loading: VLM-token
    adapters, state/action encoders, DiT, action decoder, and future torque
    decoder."""
