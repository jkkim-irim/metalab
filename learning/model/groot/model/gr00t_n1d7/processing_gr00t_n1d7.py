...
from learning.model.groot.configs.data.embodiment_configs import ModalityConfig
from learning.model.groot.data.embodiment_tags import EmbodimentTag
from learning.model.groot.data.interfaces import BaseProcessor
from learning.model.groot.data.state_action.state_action_processor import StateActionProcessor
from learning.model.groot.data.utils import (
    normalize_values_minmax,
    parse_modality_configs,
    to_json_serializable,
    unnormalize_values_minmax,
)
from learning.model.qwen3vl.pretrained_base import AutoProcessor, BatchFeature, cached_file
...
try:
    from learning.model.qwen3vl.processing import Qwen3VLProcessor
except ImportError:
...
class Gr00tN1d7Processor(BaseProcessor):
    ...
    def __init__(
        ...
        future_torque_state_key: str | None = None,
        future_torque_target_key: str | None = None,
        max_future_torque_dim: int | None = None,
        ...
        self.future_torque_state_key = future_torque_state_key
        self.future_torque_target_key = future_torque_target_key
        self.max_future_torque_dim = max_future_torque_dim or max_state_dim
        ...
        # Statistics cache (mirrors state_action_processor.statistics for aux target decoding).
    ...
    def _normalize_aux_target(
        ...
        stats = self.statistics.get(embodiment_tag.value, {}).get(modality, {}).get(key)
        if stats is None:
            raise KeyError(
                f"No normalization stats found for {modality}.{key} under embodiment "
                f"'{embodiment_tag.value}'."
            )

        min_key, max_key = ("q01", "q99") if self.use_percentiles else ("min", "max")
        params = {
            "min": np.asarray(stats[min_key]),
            "max": np.asarray(stats[max_key]),
        }
    ...
    def _unapply_aux_target(
        ...
        return unnormalize_values_minmax(values, params)

    def decode_future_torque(
        self,
        future_torque: np.ndarray,
        embodiment_tag: EmbodimentTag,
    ) -> dict[str, np.ndarray]:
        if self.future_torque_target_key is not None:
            key = self.future_torque_target_key
            value = self._unapply_aux_target(future_torque, embodiment_tag, "target", key)
            return {f"target.{key}": value.astype(np.float32)}
        if self.future_torque_state_key is not None:
            key = self.future_torque_state_key
            value = self._unapply_aux_target(future_torque, embodiment_tag, "state", key)
            return {f"state.{key}": value.astype(np.float32)}
        return {}

    def _pad_future_torque_target(self, normalized_torque: torch.Tensor):
        if normalized_torque.ndim == 1:
            normalized_torque = normalized_torque[None, :]

        torque_horizon, torque_dim = normalized_torque.shape
        if torque_horizon > self.max_action_horizon:
            raise ValueError(
                f"Future torque horizon {torque_horizon} exceeds max_action_horizon "
                f"{self.max_action_horizon}"
            )
        if torque_dim > self.max_future_torque_dim:
            raise ValueError(
                f"Future torque dim {torque_dim} exceeds max_future_torque_dim "
                f"{self.max_future_torque_dim}"
            )

        padded_torque = torch.zeros(
            self.max_action_horizon,
            self.max_future_torque_dim,
            dtype=normalized_torque.dtype,
        )
        torque_mask = torch.zeros_like(padded_torque)
        padded_torque[:torque_horizon, :torque_dim] = normalized_torque
        torque_mask[:torque_horizon, :torque_dim] = 1.0
        return padded_torque, torque_mask

    def _make_future_torque_target(self, content, embodiment_tag: EmbodimentTag):
        if self.future_torque_state_key is None and self.future_torque_target_key is None:
            return None, None

        if self.future_torque_target_key is not None:
            future_torque_targets = content.metadata.get("future_torque_targets", {})
            if self.future_torque_target_key not in future_torque_targets:
                if self.training:
                    raise KeyError(
                        f"Future torque target '{self.future_torque_target_key}' was not "
                        "provided by the dataset. Check lerobot_episode_loader target extraction."
                    )
                return None, None

            normalized_torque = self._normalize_aux_target(
                values=future_torque_targets[self.future_torque_target_key],
            ...
            if normalized_torque.shape[0] >= action_horizon + 1:
                # Dataset metadata is [current, future_1, ..., future_H].
                normalized_torque = normalized_torque[1 : 1 + action_horizon]
            else:
                normalized_torque = normalized_torque[:action_horizon]
            normalized_torque = torch.from_numpy(normalized_torque).to(torch.get_default_dtype())
            return self._pad_future_torque_target(normalized_torque)

        modality_config = self.modality_configs[embodiment_tag.value]
        if self.future_torque_state_key not in modality_config["state"].modality_keys:
            raise KeyError(
                f"future_torque_state_key='{self.future_torque_state_key}' is not one of "
                f"the configured state keys: {modality_config['state'].modality_keys}"
            )

        future_state_targets = content.metadata.get("future_state_targets", {})
        if self.future_torque_state_key not in future_state_targets:
            if self.training:
                raise KeyError(
                    f"Future state target '{self.future_torque_state_key}' was not provided by "
                    "the dataset. Check sharded_single_step_dataset.extract_step_data."
                )
            return None, None

        state_for_norm = dict(content.states)
        state_for_norm[self.future_torque_state_key] = future_state_targets[
            self.future_torque_state_key
        ]
        norm_state_dict = self.state_action_processor.apply_state(
            state=state_for_norm,
            embodiment_tag=embodiment_tag.value,
        )

        normalized_torque = torch.from_numpy(
            norm_state_dict[self.future_torque_state_key]
    ...
    def __call__(
        ...
        future_torque, future_torque_mask = self._make_future_torque_target(
            content,
            embodiment_tag,
        )
        ...
        if future_torque is not None:
            transformed_inputs["future_torque"] = future_torque
            transformed_inputs["future_torque_mask"] = future_torque_mask
        # Add VLM inputs
    ...
    def save_pretrained(self, save_directory: str | Path) -> list[Path]:
        ...
        config = {
            ...
            "processor_kwargs": {
                ...
                "future_torque_state_key": self.future_torque_state_key,
                "future_torque_target_key": self.future_torque_target_key,
                "max_future_torque_dim": self.max_future_torque_dim,
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str | Path, **kwargs):
        ...
        processor_kwargs.setdefault("future_torque_state_key", None)
        processor_kwargs.setdefault("future_torque_target_key", None)
        processor_kwargs.setdefault(
            "max_future_torque_dim",
            processor_kwargs.get("max_state_dim"),
        )
        ...
        if kwargs:
            ...
            override_keys = [
                ...
                "model_name",
                "model_type",
                "future_torque_state_key",
                "future_torque_target_key",
                "max_future_torque_dim",
            ]
