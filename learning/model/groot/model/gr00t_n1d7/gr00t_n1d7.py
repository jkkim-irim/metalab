...
from learning.model.groot.model.modules.dit import AlternateVLDiT, DiT, SelfAttentionTransformer
from learning.model.groot.model.modules.embodiment_conditioned_mlp import (
    CategorySpecificMLP,
    MultiEmbodimentActionEncoder,
)
from learning.model.qwen3vl.pretrained_base import (
    AutoConfig,
    AutoModel,
    BatchFeature,
    PreTrainedModel,
...
class Gr00tN1d7ActionHead(nn.Module):
    ...
    def __init__(self, config: Gr00tN1d7Config):
        ...
        self.future_torque_loss_weight = config.future_torque_loss_weight
        self.future_torque_dim = config.future_torque_dim or config.max_state_dim
        self.future_torque_key = config.future_torque_target_key or config.future_torque_state_key
        self.use_future_torque_loss = (
            self.future_torque_loss_weight > 0 and self.future_torque_key is not None
        )
        ...
        if self.use_future_torque_loss:
            self.future_torque_decoder = CategorySpecificMLP(
                num_categories=config.max_num_embodiments,
                input_dim=self.hidden_size,
                hidden_dim=self.hidden_size,
                output_dim=self.future_torque_dim,
            )
            self.reset_future_torque_decoder_output()
        else:
            self.future_torque_decoder = None
    ...
    def reset_future_torque_decoder_output(self) -> None:
        if getattr(self, "future_torque_decoder", None) is None:
            return
        with torch.no_grad():
            nn.init.zeros_(self.future_torque_decoder.layer2.W)
            nn.init.zeros_(self.future_torque_decoder.layer2.b)

    def reset_system1_weights(self) -> int:
        """Reinitialize System1 while keeping the VLM/backbone loaded.

        This notmulti baseline keeps the original GR00T token layout, but starts
        the motor side fresh: VLM-token adapters, state/action encoders, DiT,
        action decoder, future-torque decoder, and positional embeddings.
        """
        ...
        if self.future_torque_decoder is not None:
            modules_to_reset.append(self.future_torque_decoder)
        if self.config.add_pos_embed:
            modules_to_reset.append(self.position_embedding)
        ...
        for module in modules_to_reset:
            for parameter in module.parameters(recurse=True):
                ...
                if param_id not in seen_params:
                    ...
                    reset_param_count += parameter.numel()

            def reset_fn(child: nn.Module):
        ...
        return reset_param_count

    def _decode_future_torque_from_action_tokens(
        self,
        ...
        if not self.use_future_torque_loss or self.future_torque_decoder is None:
            return None
        action_hidden = model_output[:, -horizon:]
        future_torque_pred = self.future_torque_decoder(action_hidden, embodiment_id)
        return future_torque_pred[..., :dim]
    ...
    def set_trainable_parameters(
        ...
        if not tune_projector:
            ...
            if self.future_torque_decoder is not None:
                self.future_torque_decoder.requires_grad_(False)
            if self.config.add_pos_embed:
    ...
    def set_frozen_modules_to_eval_mode(self):
        ...
        if self.training:
            if not self.tune_projector:
                ...
                if self.future_torque_decoder is not None:
                    self.future_torque_decoder.eval()
                if self.config.add_pos_embed:
    ...
    def forward(self, backbone_output: BatchFeature, action_input: BatchFeature) -> BatchFeature:
        ...
        action_loss_scalar = action_loss.sum() / (action_mask.sum() + 1e-6)
        loss = action_loss_scalar

        future_torque_loss = None
        future_torque_weighted_loss = None
        future_torque_pred = None
        future_torque_target = action_input.get("future_torque", None)
        future_torque_mask = action_input.get("future_torque_mask", None)
        if self.use_future_torque_loss and future_torque_target is not None:
            future_torque_pred = self._decode_future_torque_from_action_tokens(
                model_output,
                embodiment_id,
                horizon=future_torque_target.shape[1],
                dim=future_torque_target.shape[-1],
            )
            assert future_torque_pred is not None
            if future_torque_mask is None:
                future_torque_mask = torch.ones_like(future_torque_target)

            finite_mask = future_torque_mask.to(dtype=torch.float32)
            finite_mask = finite_mask * torch.isfinite(future_torque_target).to(torch.float32)
            finite_mask = finite_mask * torch.isfinite(future_torque_pred).to(torch.float32)
            future_torque_pred_safe = torch.nan_to_num(
                future_torque_pred.float(), nan=0.0, posinf=1.0, neginf=-1.0
            ).clamp(-100.0, 100.0)
            future_torque_target_safe = torch.nan_to_num(
                future_torque_target.float(), nan=0.0, posinf=1.0, neginf=-1.0
            ).clamp(-100.0, 100.0)
            future_torque_loss_tensor = (
                F.mse_loss(
                    future_torque_pred_safe,
                    future_torque_target_safe,
                    reduction="none",
                )
                * finite_mask
            )
            future_torque_loss = future_torque_loss_tensor.sum() / finite_mask.sum().clamp_min(1.0)
            future_torque_weighted_loss = self.future_torque_loss_weight * future_torque_loss
            loss = loss + future_torque_weighted_loss

        outputs = {
            "loss": loss,
            "action_loss": action_loss,
            "action_loss_scalar": action_loss_scalar,
            "action_mask": action_mask,
            "backbone_features": vl_embeds,
            "state_features": state_features,
        }
        if future_torque_loss is not None:
            outputs["future_torque_loss"] = future_torque_loss
            outputs["future_torque_weighted_loss"] = future_torque_weighted_loss
            outputs["future_torque_pred"] = future_torque_pred
            outputs["future_torque_target"] = future_torque_target
            outputs["future_torque_mask"] = future_torque_mask
        return outputs
    ...
    def get_action_with_features(
        ...
        if self.use_future_torque_loss and final_model_output is not None:
            future_torque_pred = self._decode_future_torque_from_action_tokens(
                final_model_output,
                embodiment_id,
                horizon=self.action_horizon,
                dim=self.future_torque_dim,
            )
            if future_torque_pred is not None:
                output_data["future_torque_pred"] = future_torque_pred
                output_data["future_torque_key"] = self.future_torque_key
