...
try:
    from learning.model.qwen3vl.modeling import (
        Qwen3VLConfig,
...
class Qwen3Backbone(torch.nn.Module):
    def __init__(
        ...
        # Build the backbone from its CONFIG ONLY — do NOT load Cosmos's pretrained weights. A moment
        # later Gr00tN1d7.from_pretrained loads the GR00T base checkpoint, whose state_dict provides
        # every backbone weight (measured: 0 missing keys — the base covers 100% of the backbone; and
        # 477/494 tensors, ~1.835B params, differ from Cosmos: the backbone is GR00T's own fine-tuned
        # weights). So upstream GR00T's `from_pretrained(model_name)` downloads + loads Cosmos's 4.87 GB
        # of weights only to overwrite all of them. We need Cosmos for its architecture config +
        # tokenizer (~12 MB, bundled in the base checkpoint under backbone_assets/), not its weights.
        config = Qwen3VLConfig.from_pretrained(model_name)
        self.model = Qwen3VLForConditionalGeneration._from_config(
            config, attn_implementation=extra_kwargs.get("attn_implementation")
        ).eval()
        if load_bf16:
