# MetaLab 사용설명서 — 첫 학습·평가

로컬 GPU 에서 태스크 하나를 **newton** 과 **genesis** 두 엔진으로 학습·평가하는 법.

명령은 사실상 둘이다 — `metalab_train.sh`(학습), `metalab_eval.sh`(평가). 나머지 설정은 전부 계약서와
`experiment.py` 에 있다. **플래그·환경변수 전체 목록은 [`learning/scripts/local/README.md`](../../../learning/scripts/local/README.md)** 에 있고,
여기서는 처음 한 번을 돌리는 흐름만 다룬다.

## 0 · 준비

```bash
sim/metalab/setup.sh          # 핀 고정된 엔진 소스 clone + 엔진별 uv 환경 (conda 없음)
```
...
- **크로스-엔진 평가**(newton 에서 학습 → genesis 에서 평가)도 `--sim` 만 바꾸면 된다. 다만 현재
  sim2sim 교차평가 SR 은 0% 다 — **01_engine_parity** 참고.

## 5 · 확장

- **새 태스크** = 계약서만 쓴다. `contract/tasks/<name>/` 에 `_base.py`(공유 코어)와
  `<name>_<recipe>.py`(reward/gate/curriculum 트리오)를 두면 런치패드와 스크립트가 자동으로 인식한다.
  보상·관측·종료 계산식은 `terms/` 의 심볼을 조합한다. 엔진 코드는 손대지 않는다.
- **새 엔진** = 스포크만 붙인다. `backends/<engine>/{parser,backend,server}` 가 계약서를 그 엔진의 씬으로
  번역하고 공통 인터페이스(read·control·step·reset)를 구현하면, 같은 계약서가 그대로 학습된다.

## 6 · 자주 겪는 문제

| 증상 | 원인 · 해결 |
|---|---|
| `ModuleNotFoundError` | 스크립트를 직접 부르면 엔진 uv venv 가 안에서 활성화된다. 손으로 `python -m ...` 을 돌렸다면 venv 를 먼저 활성화한다. |
...
---
개요 = **00_project_overview** · 엔진 parity = **01_engine_parity** · 런치패드 = **20_launchpad_prd**
