...
def _coupled_pd_oracle_check(group_name, joints, model_file, poly, k_phi, k_d, seed, gain_slice=None):
    ...
    from sim.metalab.control.motor.motor_coupling import MotorCoupledPDHand, load_hand_group
    wp.init()
...
def _coupled_pd_arm_oracle_check(joints, json_path, arm_slice, seed):
    ...
    from sim.metalab.control.motor.motor_coupling import MotorCoupledPDArm, load_arm_group
    wp.init()
...
def test_coupled_pd_gravcomp_folds_into_motor_clamp():
    ...
    from sim.metalab.control.motor.motor_coupling import (
        MotorCoupledPDArm,
        MotorCoupledPDHand,
        load_arm_group,
        load_hand_group,
...
def test_shared_finger_map_backs_multiple_groups():
    ...
    try:
        from sim.metalab.control.motor.motor_coupling import load_hand_group
    except ModuleNotFoundError:
