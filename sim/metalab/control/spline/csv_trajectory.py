...
A ``CsvTrajectory`` reads a directory of via-point CSVs, builds one dense, time-uniform
joint-target buffer [rad] and hands out one target row per :meth:`step` call. WHICH file
drives WHICH joints comes from the robot contract (``spec.robot.csv_spline_groups()``:
CSV basename -> joint names in column order), so any robot plays a group directory that
follows the format — no robot is named here. It is **pure** (numpy only, no
engine/backend/torch import): the runner reads :attr:`joint_names` + :meth:`step` and
writes the row into ``backend.set_joint_targets``.
...
class CsvTrajectory:
    ...
    Args:
        csv_dir: directory holding the ``*.csv`` via-point files (a subset of ``groups`` —
            missing files are skipped).
        groups: CSV basename (no ``.csv``) -> joint names IN COLUMN ORDER, from
            ``spec.robot.csv_spline_groups()``.
    ...
    def __init__(self, csv_dir, groups, hz: float, *,
        ...
        # 1) parse every group the directory actually ships
        parsed = []          # (csv_name, ViaCSVData, [joint_name, ...])
        durations: list[tuple[str, float]] = []
        for csv_name, jnames in groups.items():
            ...
            assert n_cols == len(jnames), (
                f"{csv_name}.csv has {n_cols} joint cols but robot group {csv_name!r} has "
                f"{len(jnames)} joints — CSV/robot mismatch")
            parsed.append((csv_name, data, list(jnames)))
            durations.append((csv_name, float(data.t_via[-1])))

        assert parsed, (
            f"no trajectory CSV of this robot's spline groups found in {self._dir} — "
            f"expected one of {sorted(f'{n}.csv' for n in groups)}")
        ...
        self.groups_used = [g[0] for g in parsed]
        ...
        if self.max_duration_s == 0.0:
            reach = max((data.reach0 for _, data, _ in parsed), default=0.0)
            if reach > 0.0:
                ramp_s = reach
