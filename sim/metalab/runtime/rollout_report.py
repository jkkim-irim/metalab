...
Pairs the ``.rrd`` rerun recording the sim writes (``backends/newton/viewer_rerun_sink``) with the series
:class:`sim.metalab.runtime.rollout_log.PerEnvRolloutLog` records, so a checkpoint can be *inspected* and
...
Usage (also the wiring point for the eval/train paths)::
    ...
    python -m sim.metalab.runtime.rollout_report <record_dir> [--out FILE] [--title TEXT]
"""
...
/* Same two-combo channel picker as the Launchpad's RL tab (drive/dashboard_page.py #psec/#pch): section
...
// --- sync contract (see runtime/rollout_log.py): sample index <-> wall time -----------------------
...
def build_report(record_dir: str, out_path: str | None = None, title: str = "") -> str:
    ...
    assert os.path.exists(data_path), (
        ...
        f"recording (runtime/rollout_log.py); was this recording made before that existed?")
